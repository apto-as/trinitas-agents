...
class SettingsMerger:
        ...
        for hook_type, entries in hooks.items():
            for entry in entries:
                for hook in entry.get("hooks", []):
                    if hook.get("type") == "command" and ".py" in hook.get(
                        "command", ""
                    ):
                        if "environment" not in hook or "PYTHONPATH" not in hook.get(
                            "environment", {}
                        ):
                            conflicts.append(
    ...
    def validate_merged_settings(self, merged: Dict[str, Any]) -> List[str]:
        ...
        if "PreToolUse" in hooks:
            ...
            if len(task_hooks) > 1:
                ...
                if not any(
                    "prepare_parallel_tasks.py" in h.get("command", "")
                    for h in first_hooks
                ):
                    issues.append(
    ...
    def write_merged_settings(
        self, merged: Dict[str, Any], output_path: str = None
    ) -> Path:
...
def main():
    ...
    parser.add_argument("settings_path", help="Path to existing settings.json")
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force merge even with conflicts"
    )
