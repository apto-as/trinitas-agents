...
class AutoRecoverySystem:
    ...
    def __init__(self):
        ...
        self.results_dir = Path(
            os.environ.get(
                "TRINITAS_RESULTS_DIR", self.trinitas_dir / "parallel_results"
            )
        )
        ...
        for session_id in issue["sessions"]:
            try:
                ...
                if result.returncode == 0:
                    ...
                    self.logger.error(
                        f"Integration failed for {session_id}: {result.stderr}"
                    )
            ...
            for subdir in ["completed", "integrated", "failed"]:
                ...
                for item in dir_path.iterdir():
                    if item.stat().st_mtime < cutoff_time:
                        ...
                        freed_mb += size_mb
                        self.logger.info(
                            f"Removed old item: {item.name} ({size_mb:.1f}MB)"
                        )
    ...
    def run_recovery(self, auto_fix: bool = False) -> Dict:
        ...
        # Save report
        report_file = (
            self.log_dir / f"recovery_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        with open(report_file, "w") as f:
            json.dump(report, f, indent=2)
...
def main():
    ...
    parser.add_argument(
        "--auto-fix", action="store_true", help="Automatically attempt to fix issues"
    )
    parser.add_argument(
        "--diagnose-only",
        action="store_true",
