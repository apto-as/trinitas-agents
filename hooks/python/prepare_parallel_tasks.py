...
class ParallelTaskPreparer:
    ...
    def analyze_prompt(self) -> Dict[str, Any]:
        ...
        if analysis['complexity'] >= 3 and len(analysis['suitable_agents']) >= 2:
            ...
            analysis['estimated_speedup'] = min(len(analysis['suitable_agents']) * 0.7, 2.5)
    ...
    def _assess_complexity(self) -> int:
        ...
        if any(keyword in self.user_prompt.lower() for keyword in multi_aspect_keywords):
            complexity += 1
        
        # Technical depth indicators
        technical_keywords = ['optimize', 'analyze', 'security', 'architecture', 'performance']
        if sum(1 for keyword in technical_keywords if keyword in self.user_prompt.lower()) >= 2:
            complexity += 1
    ...
    def _identify_suitable_agents(self) -> List[str]:
        ...
        for agent, config in AGENT_CAPABILITIES.items():
            ...
            strength_matches = sum(1 for strength in config['strengths']
    ...
    def _create_agent_task(self, agent_type: str) -> Optional[Dict[str, Any]]:
        ...
        config = AGENT_CAPABILITIES[agent_type]
...
def generate_hook_output(tasks: List[Dict[str, Any]], session_id: str) -> Dict[str, Any]:
...
def main():
    ...
    preparer = ParallelTaskPreparer(user_prompt, {
