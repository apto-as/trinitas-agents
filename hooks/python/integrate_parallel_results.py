...
class ParallelResultIntegrator:
    ...
    def integrate_results(self):
        ...
        for agent_type, agent_results in by_agent.items():
            self.integrated_result["agent_results"][agent_type] = {
                ...
                "execution_time_ms": sum(
                    r.get("execution_time_ms", 0) for r in agent_results
                ),
        ...
        self.integrated_result["metadata"] = {
            ...
            "total_execution_time_ms": sum(
                r.get("execution_time_ms", 0) for r in self.results
            ),
        }
    ...
    def _merge_agent_results(self, agent_type: str, results: List[Dict]) -> Dict:
        ...
        merged = {"content": [], "errors": [], "duplicates_removed": 0}
    ...
    def _synthesize_results(self) -> Dict:
        ...
        for agent_type, data in self.integrated_result["agent_results"].items():
            ...
            if content:
                ...
                if agent_type == "vector-auditor" and "risk" in content.lower():
                    ...
                    insights.append(
                        f"Optimization opportunity identified by {agent_type}"
                    )

        synthesis["key_insights"] = insights[:5]  # Top 5 insights
        synthesis["summary"] = (
            f"Integrated results from {len(self.integrated_result['agent_results'])} agents"
        )
        ...
        if len(self.integrated_result["agent_results"]) > 1:
            ...
            error_count = sum(
                1
                for a in self.integrated_result["agent_results"].values()
                if a["status"] != "success"
            )
            if error_count == 0:
    ...
    def _create_summary_markdown(self, output_file: Path):
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            f.write("# Parallel Execution Summary\n\n")
            f.write(f"**Session ID**: {self.session_id}\n")
            f.write(f"**Timestamp**: {self.integrated_result['timestamp']}\n")
            f.write(
                f"**Total Execution Time**: {self.integrated_result['metadata']['total_execution_time_ms']}ms\n\n"
            )
            ...
            for agent, data in self.integrated_result["agent_results"].items():
                ...
                f.write(
                    f"- Results: {len(data['results'].get('content', ''))} characters\n\n"
                )
...
def main():
    ...
    if not session_id:
        ...
        print(
            "   or: TRINITAS_INTEGRATION_SESSION=<session_id> integrate_parallel_results.py"
        )
        return 1
