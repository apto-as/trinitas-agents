                ...
                if isinstance(node, ast.Assign):
                    for target in node.targets:
                        if isinstance(target, ast.Name):
                            ...
                            if any(secret in name for secret in ["password", "token", "key", "secret"]):
    ...
    def _analyze_javascript(self, file_path: str):
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                # Hardcoded tokens
                if any(pattern in line.lower() for pattern in ["api_key", "apikey", "token", "secret"]):
                ...
                for func in self.DANGEROUS_FUNCTIONS[language]:
                    for i, line in enumerate(lines, 1):
                        if func in line and not line.strip().startswith(("#", "//")):
                            self.vulnerabilities.append(CodeIssue(
                                file=file_path,
                                line=i,
