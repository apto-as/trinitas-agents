...
class QualityChecker:
    ...
    def _calculate_metrics(self, file_path: str) -> QualityMetrics:
        ...
        try:
            ...
            logging.warning(f"Failed to analyze quality metrics for {file_path}: {e}")
            return QualityMetrics(
                file_path=file_path,
                lines_of_code=0,
    ...
    def _check_python_quality(self, file_path: str) -> List[CodeIssue]:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if re.search(r"\b\d{2,}\b", line) and not re.search(
                    r"(#|//|\"|\').*\d{2,}", line
                ):
                    issues.append(
                        CodeIssue(
            ...
            logging.warning(f"Failed to check Python quality for {file_path}: {e}")
            return issues
    ...
    def _check_universal_quality(self, file_path: str) -> List[CodeIssue]:
        ...
        try:
            ...
            logging.warning(f"Failed to check nesting level for {file_path}: {e}")
