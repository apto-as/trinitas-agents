...
class HookResult:
    ...
    def to_shell_format(self) -> str:
        ...
        output = f"{color}{symbol} Hook {self.status.value.title()}{reset}: {self.message}"
...
class TrinitasHook(ABC):
    ...
    def __init__(self):
        ...
        self.tool_name = os.environ.get("CLAUDE_TOOL_NAME", "")
        self.tool_args_raw = os.environ.get("CLAUDE_TOOL_ARGUMENTS", "{}")
