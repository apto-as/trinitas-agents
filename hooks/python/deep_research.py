...
class DeepResearchEngine:
    ...
    async def conduct_deep_research(
        self, initial_query: ResearchQuery
    ) -> Dict[str, Any]:
        """深層研究を実施"""
        ...
        root_node = ResearchNode(
            node_id=f"node_root_{tree_id}", query=initial_query, depth=0, parent_id=None
        )
        self.research_trees[tree_id][root_node.node_id] = root_node

        # 選択された戦略で研究を実行
        strategy_func = self.strategies.get(
            initial_query.strategy, self._hybrid_strategy
        )
    ...
    async def _depth_first_strategy(self, tree_id: str, root_node: ResearchNode):
        ...
        async def dfs(node_id: str):
            ...
            if child_queries:
                ...
                child_node = await self._create_child_node(tree_id, node, best_query)
    ...
    async def _hybrid_strategy(self, tree_id: str, root_node: ResearchNode):
        ...
        promising_nodes = sorted(
            level1_nodes, key=lambda n: n.quality_score, reverse=True
        )[:3]  # 上位3つ
        ...
        async with AsyncMCPClient() as client:
            ...
            if "academic" in query.context.get("domains", []):
                requests.append(
                    MCPRequest(
                        ...
                        server="arxiv",
                        method="search",
                        params={"query": query.question, "max_results": 10},
                    )
                )
            ...
            requests.append(
                MCPRequest(
                    ...
                    server="web_search",
                    method="search",
                    params={"query": query.question, "max_results": 5},
                )
            )
    ...
    async def _generate_child_queries(
        ...
        for finding in parent_node.findings:
            ...
            derived_questions = self._derive_questions(
                parent_node.query.question, finding
            )
    ...
    async def _create_child_node(
        self, tree_id: str, parent_node: ResearchNode, query: ResearchQuery
    ) -> ResearchNode:
        """子ノードを作成"""
    ...
    async def _explore_at_depth(
        self, tree_id: str, root_node: ResearchNode, target_depth: int
    ):
        """特定の深さで探索"""
    ...
    async def _aggregate_research_results(self, tree_id: str) -> Dict[str, Any]:
        ...
        # 最良のパスを選択
        best_paths = sorted(
            paths.values(), key=lambda p: p.total_quality, reverse=True
        )[:5]
        ...
        for node in tree.values():
            if node.status == "completed":
                ...
                high_quality_findings = [
                    f for f in node.findings if f.get("quality_score", 0) > 0.7
                ]
        ...
        return {
            "tree_id": tree_id,
            "total_nodes_explored": len(
                [n for n in tree.values() if n.status == "completed"]
            ),
            "max_depth_reached": max(n.depth for n in tree.values()),
            "total_findings": len(all_findings),
            "high_quality_findings": len(
                [f for f in all_findings if f.get("quality_score", 0) > 0.7]
            ),
    ...
    def _derive_questions(
        self, parent_question: str, finding: Dict[str, Any]
    ) -> List[str]:
        """発見から新しい質問を導出"""
        questions = []
    ...
    def _calculate_query_priority(
        self, question: str, parent_finding: Dict[str, Any]
    ) -> int:
        """クエリの優先度を計算"""
        priority = 5  # デフォルト
    ...
    def _extract_research_paths(self, tree_id: str) -> Dict[str, ResearchPath]:
        ...
        for leaf in leaf_nodes:
            ...
            # パスの品質を計算
            path_quality = sum(tree[nid].quality_score for nid in path_nodes) / len(
                path_nodes
            )
...
async def execute_deep_research(
    topic: str, questions: List[str], domains: List[str], max_depth: int = 3
) -> Dict[str, Any]:
    """深層研究を実行（外部インターフェース）"""
    ...
    initial_query = ResearchQuery(
        ...
        question=topic,
        context={"domains": domains, "sub_questions": questions},
        priority=10,
        max_depth=max_depth,
