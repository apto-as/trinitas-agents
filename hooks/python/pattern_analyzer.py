...
class PatternAnalyzer:
    ...
    def _load_relations(self) -> List[PatternRelation]:
        ...
        if self.relations_db.exists():
            with open(self.relations_db) as f:
                ...
                relations = []
                for rdata in data:
                    relations.append(PatternRelation(
    ...
    def analyze_execution_history(self, history: List[Dict[str, Any]]) -> Dict[str, Any]:
        """実行履歴を分析してパターンを抽出"""
        analysis_results = {
            "sequence_patterns": [],
    ...
    def _analyze_sequences(self, history: List[Dict[str, Any]]) -> List[ExtractedPattern]:
        """シーケンシャルパターンを分析"""
        sequences = []
        ...
        for i in range(len(history) - self.sequence_window + 1):
            ...
            existing = next(
                (s for s in sequences if "_".join(s.elements) == sequence_key),
                None
            )
    ...
    def _analyze_correlations(self, history: List[Dict[str, Any]]) -> List[ExtractedPattern]:
        """相関パターンを分析"""
        correlations = []
    ...
    def _analyze_frequencies(self, history: List[Dict[str, Any]]) -> List[ExtractedPattern]:
        """頻度パターンを分析"""
        frequencies = []
    ...
    def _detect_anomalies(self, history: List[Dict[str, Any]]) -> List[ExtractedPattern]:
        """異常パターンを検出"""
        anomalies = []
        ...
        if exec_times:
            ...
            std_dev = (sum((t - avg_time) ** 2 for t in exec_times) / len(exec_times)) ** 0.5
        ...
        for i, entry in enumerate(history):
            if not entry.get("outcome", {}).get("success", True):
                ...
                if consecutive_failures >= 3:
                    # 3回以上の連続失敗を異常とする
                    failure_entries = history[failure_start:failure_start + consecutive_failures]
                    pattern = ExtractedPattern(
                        pattern_type=PatternType.ANOMALY,
    ...
    def _find_optimizations(self, history: List[Dict[str, Any]]) -> List[ExtractedPattern]:
        """最適化パターンを発見"""
        optimizations = []
    ...
    def _analyze_pattern_relations(self, analysis_results: Dict[str, Any]):
        ...
        for i, pattern1 in enumerate(all_patterns):
            for pattern2 in all_patterns[i+1:]:
                # 時系列的な関係
                if (pattern1.pattern_type == PatternType.SEQUENCE and 
                    pattern2.pattern_type == PatternType.SEQUENCE):
                    # シーケンスの連続性をチェック
                ...
                # 相関関係
                if (pattern1.pattern_type == PatternType.CORRELATION and
                    pattern2.pattern_type == PatternType.OPTIMIZATION):
                    # 同じコンテキストでの最適化
                    if pattern1.context.get("task_type") == pattern2.context.get("task_type"):
                        relation = PatternRelation(
                            pattern1_id=self._generate_pattern_id(pattern1),
                            pattern2_id=self._generate_pattern_id(pattern2),
    ...
    def _generate_pattern_id(self, pattern: ExtractedPattern) -> str:
        ...
        content = f"{pattern.pattern_type.value}_{pattern.description}_{pattern.elements}"
    ...
    def find_similar_patterns(self, target_pattern: Dict[str, Any]) -> List[Tuple[str, float]]:
        """類似パターンを検索"""
        similarities = []
                    ...
                    "similar_patterns": [
                        {
                            ...
                            } if pid in analyzer.patterns else None
                        }
