...
class EnhancedQualityCheckHook(TrinitasHook):
    ...
    def run(self) -> HookResult:
        ...
        if self.tool_name not in ["Write", "Edit", "MultiEdit"]:
            return HookResult(
                status=HookStatus.SUCCESS, message="Not applicable to this tool"
            )
        ...
        if not file_path:
            return HookResult(
                status=HookStatus.WARNING, message="Could not extract file path"
            )

        # Check if file exists
        if not os.path.exists(file_path):
            return HookResult(
                status=HookStatus.WARNING, message=f"File not found: {file_path}"
            )

        self.logger.krukai(
            "Python強化版の品質チェックを開始するわ。妥協は許さない", "Quality Check"
        )
        ...
        if error_count > 0:
            self.logger.vector(
                f"……{error_count}個の重大な問題を検出。即座に修正が必要……"
            )
            return HookResult(
            ...
            self.logger.krukai(
                f"フン、{warning_count}個の警告があるわね。改善の余地があるわ"
            )
            return HookResult(
    ...
    def _generate_report(self, file_path: str, issues: List[Any], metrics: Optional[Any]) -> None:
    ...
    def _format_issues_summary(self, issues: List[Any]) -> str:
