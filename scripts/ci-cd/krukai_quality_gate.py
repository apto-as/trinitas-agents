...
class KrukaiQualityGate:
    ...
    def _analyze_python_quality(self, files: List[Path]) -> float:
        ...
        try:
            ...
            pylint_result = subprocess.run(
                ["pylint", "--output-format=json"] + [str(f) for f in files[:10]],  # Limit files
                capture_output=True,
                text=True,
            ...
            if pylint_result.returncode in [0, 4, 8, 16]:  # Various pylint exit codes
                try:
                    pylint_data = json.loads(pylint_result.stdout) if pylint_result.stdout.strip() else []
    ...
    def _analyze_js_quality(self, files: List[Path]) -> float:
        ...
        has_modules = any('import' in f.read_text(errors='ignore') or 'require(' in f.read_text(errors='ignore') 
                         for f in files[:5])
        if has_modules:
            score += 10
    ...
    def _analyze_general_quality(self) -> float:
        ...
        test_files = list(self.project_root.rglob("*test*.py")) + list(self.project_root.rglob("*.test.js"))
    ...
    def run_security_quality_check(self) -> float:
        ...
        env_usage = any('os.environ' in file_path.read_text(errors='ignore') or 'process.env' in file_path.read_text(errors='ignore')
                       for file_path in code_files[:10] if file_path.stat().st_size < 100000)
        if env_usage:
            score += 15
    ...
    def generate_krukai_assessment(self) -> Dict:
        ...
        grade = "S" if overall_score >= 95 else \
                "A" if overall_score >= 85 else \
                "B" if overall_score >= 75 else \
                "C" if overall_score >= 65 else "F"
