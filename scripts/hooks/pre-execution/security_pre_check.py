...
class TrinitasSecurityPreCheck:
    ...
    def validate_content_security(self, content: str, file_path: str) -> Dict:
        ...
        applicable_risks = self.file_type_risks.get(file_ext, list(self.critical_security_patterns.keys()))
        ...
        for risk_type in applicable_risks:
            if risk_type in self.critical_security_patterns:
                ...
                for pattern in patterns:
                    matches = self._find_security_issues(content, pattern, risk_type, file_path)
    ...
    def _find_security_issues(self, content: str, pattern: str, risk_type: str, file_path: str) -> List[Dict]:
            ...
            for line_num, line in enumerate(lines, 1):
                if compiled_pattern.search(line):
                    ...
                    issues.append({
                        ...
                        'educational_content': self.security_education.get(risk_type, {}),
                    })
        except re.error:
    ...
    def _determine_severity(self, risk_type: str, code_line: str) -> str:
        ...
        if any(keyword in code_line.lower() for keyword in ['production', 'prod', 'live']):
    ...
    def validate_file_operation_security(self, tool_name: str, tool_input: Dict) -> Dict:
        """
...
def main():
    ...
    try:
        ...
        if len(sys.argv) < 2:
            print(json.dumps({"allowed": True, "message": "No command provided for security pre-check"}))
            sys.exit(0)
        ...
        # Validate security
        security_result = pre_checker.validate_file_operation_security(tool_name, tool_input)
