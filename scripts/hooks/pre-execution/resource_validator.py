...
class TrinitasResourceValidator:
    ...
    def get_system_resources(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "io": {"wait_percent": io_wait, "counters": io_counters},
                "timestamp": time.time(),
            }
    ...
    def classify_operation(self, command: str, context: Dict) -> str:
        ...
        if any(
            pattern in command_lower
            for pattern in ["mysql", "postgres", "mongo", "redis", "sqlite"]
        ):
            return "database"

        if any(
            pattern in command_lower
            for pattern in ["curl", "wget", "ssh", "scp", "rsync"]
        ):
            return "network"

        if any(
            pattern in command_lower
            for pattern in ["grep", "find", "awk", "sed", "sort", "analyze"]
        ):
            return "analysis"

        if tool_name in ["write", "edit", "multiedit"] or any(
            pattern in command_lower for pattern in ["cp", "mv", "mkdir", "touch"]
        ):
    ...
    def validate_resources(
        ...
        if (
            ...
            >= self.resource_thresholds["memory_critical"]
        ):
            warnings.append(
                f"CRITICAL: Memory usage at {resources['memory']['used_percent']:.1f}% - System may become unstable"
            )
            risk_level = "CRITICAL"
            decision = "BLOCK"

        if (
            resources["disk"]["used_percent"]
            >= self.resource_thresholds["disk_critical"]
        ):
            warnings.append(
                f"CRITICAL: Disk usage at {resources['disk']['used_percent']:.1f}% - Risk of disk full"
            )
            risk_level = "CRITICAL"
            decision = "BLOCK"
        ...
        if resources["cpu"]["load_avg_1m"] >= self.resource_thresholds["load_critical"]:
            warnings.append(
                f"CRITICAL: System load at {resources['cpu']['load_avg_1m']:.1f} - System overloaded"
            )
            risk_level = "CRITICAL"
            decision = "BLOCK"
        ...
        if (
            resources["memory"]["used_percent"]
            >= self.resource_thresholds["memory_warning"]
        ):
            warnings.append(
                f"WARNING: High memory usage ({resources['memory']['used_percent']:.1f}%) - Performance may be affected"
            )
            if risk_level == "SAFE":
                risk_level = "MEDIUM"
                decision = "WARN"
        ...
        if (
            resources["disk"]["used_percent"]
            >= self.resource_thresholds["disk_warning"]
        ):
            warnings.append(
                f"WARNING: Low disk space ({resources['disk']['free_gb']:.1f}GB free) - Consider cleanup"
            )
            if risk_level == "SAFE":
                risk_level = "MEDIUM"
                decision = "WARN"
        ...
        if resources["cpu"]["usage_percent"] >= self.resource_thresholds["cpu_warning"]:
            warnings.append(
                f"WARNING: High CPU usage ({resources['cpu']['usage_percent']:.1f}%) - System may be slow"
            )
            if risk_level == "SAFE":
                risk_level = "LOW"
                decision = "WARN"
        ...
        if (
            resources["swap"]["used_percent"]
            >= self.resource_thresholds["swap_warning"]
        ):
            warnings.append(
                f"INFO: Swap memory in use ({resources['swap']['used_percent']:.1f}%) - Consider adding RAM"
            )
            if risk_level == "SAFE":
                risk_level = "LOW"
                decision = "WARN"
        ...
        if operation_type in self.operation_requirements:
            ...
            if (
                "memory" in req
                and resources["memory"]["available_gb"] * 1024 < req["memory"]
            ):
                warnings.append(
                    f"INFO: {operation_type} typically requires {req['memory']}MB RAM, {resources['memory']['available_gb'] * 1024:.0f}MB available"
                )

            if "disk" in req and resources["disk"]["free_gb"] * 1024 < req["disk"]:
                warnings.append(
                    f"INFO: {operation_type} may need {req['disk']}MB disk space, {resources['disk']['free_gb'] * 1024:.0f}MB available"
                )
...
def main():
    ...
    try:
        ...
        if len(sys.argv) < 2:
            print(
                json.dumps(
                    {
                        "allowed": True,
                        "message": "No command provided for resource validation",
                    }
                )
            )
            sys.exit(0)
        ...
        response = {
            ...
            "command": command,
            "trinitas_analysis": {
                "springfield": "Resource assessment complete - user guidance prioritized",
