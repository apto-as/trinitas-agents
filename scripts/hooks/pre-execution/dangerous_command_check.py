...
class TrinitasDangerousCommandDetector:
    ...
    def analyze_command(
        ...
        for pattern in self.critical_patterns:
            if re.search(pattern, command_lower, re.IGNORECASE):
                warning_msg = "CRITICAL: Potentially destructive command detected"
                if "rm" in command_lower and ("/" in command or "*" in command):
                    warning_msg += " - File deletion risk detected"
                elif "sudo" in command_lower:
                    warning_msg += " - Privilege escalation detected"
                elif "curl" in command_lower and "|" in command:
                    warning_msg += " - Remote code execution risk detected"
        ...
        for pattern in self.warning_patterns:
            if re.search(pattern, command_lower, re.IGNORECASE):
                warning_msg = "MEDIUM: Potentially risky command - please review"
                if "rm" in command_lower:
                    warning_msg += " - File deletion detected"
                elif "sudo" in command_lower:
                    warning_msg += " - Administrative privileges required"
                elif "chmod" in command_lower:
                    warning_msg += " - Permission changes detected"
...
def main():
    ...
    try:
        ...
        if risk_level in ["CRITICAL", "HIGH", "MEDIUM"]:
            response["safety_guidance"] = detector.generate_safety_guidance(
                command, risk_level
            )
