...
class TrinitasKnowledgePersister:
    ...
    def __init__(self):
        ...
        self.session_knowledge_file = (
            self.knowledge_base_dir / "session-knowledge.jsonl"
        )
        self.persistent_knowledge_file = (
            self.knowledge_base_dir / "persistent-knowledge.jsonl"
        )
    ...
    def extract_knowledge(
        ...
        if tool_name in ["Write", "Edit", "MultiEdit"]:
            knowledge_items.extend(
                self._extract_code_knowledge(tool_input, tool_response)
            )
        elif tool_name == "Bash":
            knowledge_items.extend(
                self._extract_command_knowledge(tool_input, tool_response)
            )
        elif tool_name in ["Grep", "Glob"]:
            knowledge_items.extend(
                self._extract_search_knowledge(tool_input, tool_response)
            )
        elif tool_name == "Task":
            knowledge_items.extend(
                self._extract_task_knowledge(tool_input, tool_response)
            )
        ...
        return {
            ...
            "categories_covered": list(
                set(item["category"] for item in categorized_knowledge)
            ),
            "total_value_score": sum(
                item["value_score"] for item in categorized_knowledge
            ),
        }

    def _extract_code_knowledge(
        self, tool_input: Dict, tool_response: Dict
    ) -> List[str]:
        ...
        if "content" in tool_input:
            ...
            if "class " in content:
                knowledge.append(
                    f"Class definition pattern: {self._extract_class_pattern(content)}"
                )

            if "def " in content:
                knowledge.append(
                    f"Function pattern: {self._extract_function_pattern(content)}"
                )
            ...
            if "try:" in content:
                knowledge.append(
                    "Error handling pattern: try-except block implementation"
                )

            # Look for async patterns
            if "async def" in content or "await " in content:
                knowledge.append(
                    "Async programming pattern: async/await implementation"
                )
        ...
        if "file_path" in tool_input:
            ...
            if file_ext in [".py", ".js", ".ts", ".java", ".go"]:
                knowledge.append(
                    f"File structure pattern: {file_ext} file in {Path(file_path).parent}"
                )

        return knowledge

    def _extract_command_knowledge(
        self, tool_input: Dict, tool_response: Dict
    ) -> List[str]:
    ...
    def _extract_task_knowledge(
        ...
        if "prompt" in tool_input:
            ...
            if any(
                word in prompt.lower() for word in ["debug", "fix", "error", "issue"]
            ):
                knowledge.append(f"Problem-solving approach: {prompt[:100]}...")

            if any(
                word in prompt.lower()
                for word in ["optimize", "improve", "performance"]
            ):
    ...
    def _score_knowledge_value(self, knowledge_item: str, category: str) -> float:
        ...
        if any(
            word in knowledge_item.lower()
            for word in ["pattern", "best practice", "solution"]
        ):
            base_score += 0.2
    ...
    def persist_execution_knowledge(
        self, tool_name: str, tool_input: Dict, tool_response: Dict, context: Dict
    ) -> Dict:
        """
        ...
        knowledge_data = self.extract_knowledge(
        ...
        return {
            ...
            "trinitas_analysis": {
                ...
                "vector": "Secure storage validated - all sensitive data sanitized",
            },
        }
