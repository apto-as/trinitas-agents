...
class TrinitasSecurityScanner:
    ...
    def __init__(self):
        ...
        self.file_security_rules = {
            '.py': ['sql_injection', 'command_injection', 'hardcoded_secrets', 'insecure_crypto'],
            '.js': ['xss_vulnerability', 'hardcoded_secrets', 'cors_misconfiguration'],
            '.php': ['sql_injection', 'xss_vulnerability', 'path_traversal', 'file_upload_issues'],
    ...
    def scan_file_security(self, file_path: str) -> Dict:
        ...
        try:
            ...
            applicable_patterns = self.file_security_rules.get(file_ext, list(self.vulnerability_patterns.keys()))
    ...
    def _find_pattern_matches(self, content: str, pattern: str, file_path: str) -> List[Dict]:
    ...
    def _perform_context_analysis(self, content: str, file_path: str) -> List[Dict]:
        ...
        security_todos = re.findall(r'(?i)(?:todo|fixme|hack|xxx).*(?:security|auth|password|crypto)', content, re.IGNORECASE)
        if security_todos:
            additional_issues.append({
                'type': 'security_todo',
                'severity': 'LOW',
    ...
    def scan_execution_security(self, tool_name: str, tool_input: Dict, 
...
def main():
    ...
    try:
        ...
        if security_result.get('critical_vulnerabilities', 0) > 0 or security_result.get('high_vulnerabilities', 0) > 0:
