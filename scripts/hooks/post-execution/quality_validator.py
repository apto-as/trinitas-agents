...
class TrinitasQualityValidator:
            ...
            return {
                ...
                "issues": {
                    ...
                    "long_functions": len(
                        [
                            f
                            for f, l in function_lengths
                            if l > self.quality_thresholds["function_length_max"]
                        ]
                    ),
                    "long_classes": len(
                        [
                            c
                            for c, l in class_lengths
                            if l > self.quality_thresholds["class_length_max"]
                        ]
                    ),
    ...
    def _calculate_quality_score(
        ...
        if comment_ratio < self.quality_thresholds["comment_ratio_min"]:
            score -= (
                20
                * (self.quality_thresholds["comment_ratio_min"] - comment_ratio)
                / self.quality_thresholds["comment_ratio_min"]
            )

        # Complexity penalty
        if complexity > self.quality_thresholds["complexity_warning"]:
            score -= min(
                30, (complexity - self.quality_thresholds["complexity_warning"]) * 2
            )
    ...
    def _generate_recommendations(
        self, comment_ratio: float, complexity: int, smells: Dict, long_lines: List
    ) -> List[str]:
        ...
        if comment_ratio < self.quality_thresholds["comment_ratio_min"]:
            recommendations.append(
                f"Add more comments - current ratio {comment_ratio:.2f}, target {self.quality_thresholds['comment_ratio_min']:.2f}"
            )

        if complexity > self.quality_thresholds["complexity_critical"]:
            recommendations.append(
                f"CRITICAL: Reduce complexity from {complexity} to below {self.quality_thresholds['complexity_critical']}"
            )
        elif complexity > self.quality_thresholds["complexity_warning"]:
            recommendations.append(
                f"Consider refactoring to reduce complexity from {complexity} to below {self.quality_thresholds['complexity_warning']}"
            )

        if long_lines:
            recommendations.append(
                f"Break {len(long_lines)} long lines (max {self.quality_thresholds['line_length_max']} chars)"
            )

        for smell, count in smells.items():
            if count > 0:
                recommendations.append(
                    f"Address {count} instances of {smell.replace('_', ' ')}"
                )
            ...
            if "error" not in analysis:
                ...
                if analysis["quality_score"] < 50:
                    critical_issues.append(
                        f"{file_path}: Quality score {analysis['quality_score']}/100"
                    )

                if (
                    analysis["metrics"]["complexity_score"]
                    > self.quality_thresholds["complexity_critical"]
                ):
                    critical_issues.append(
                        f"{file_path}: Critical complexity level {analysis['metrics']['complexity_score']}"
                    )
        ...
        if len(analyses) > 1:
            ...
            if avg_score < 70:
                unique_recommendations.insert(
                    0,
                    "Consider implementing code quality standards and review processes",
                )
