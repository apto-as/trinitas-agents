...
class TrinitasHooksInstaller:
    ...
    def __init__(self):
        ...
        self.template_file = (
            self.hooks_dir / "settings" / "claude-code-hooks-template.json"
        )
    ...
    def check_prerequisites(self) -> Tuple[bool, List[str]]:
        ...
        if (
            ...
            and not (Path.cwd() / "package.json").exists()
        ):
            issues.append(
                "Not in a recognized project directory (no .git or package.json found)"
            )
        ...
        if not self.trinitas_docs_source.exists():
            issues.append(
                f"Trinitas documentation not found: {self.trinitas_docs_source}"
            )
    ...
    def _create_settings_backup(self, settings_file: Path) -> bool:
        """
        Springfield式思いやり設計: 既存設定を安全にバックアップ
        """
        try:
            if not settings_file.exists():
                print("ℹ️  No existing settings file - backup not needed")
                return True

            # Create backup directory with timestamp
            backup_dir = settings_file.parent / "settings_backup"
            ...
            # Create backup manifest for easy restoration
            manifest = {
                "timestamp": timestamp,
                "original_file": str(settings_file),
                "backup_file": str(backup_file),
                "created_by": "Springfield-Trinitas-Hooks-Installer",
            }
    ...
    def restore_settings_from_backup(
        self, install_location: str, backup_timestamp: str = None
    ) -> bool:
        """
        Springfield式緊急復旧: バックアップから設定を復元
        """
        try:
            if install_location == "user":
            ...
            if backup_timestamp is None:
                ...
                backup_file = backup_dir / f"settings_backup_{backup_timestamp}.json"
    ...
    def install_hooks_config(self, config: Dict, install_location: str) -> bool:
        """
        Install hooks configuration to Claude Code settings - WITH MANDATORY BACKUP
        """
        ...
        # MANDATORY: Create backup before any modification
        if not self._create_settings_backup(settings_file):
            print("🛑 Installation ABORTED: Cannot proceed without backup")
            return False
...
""")
        ...
        while True:
            choice = (
                input(
                    "\nEnter your choice (minimal/standard/comprehensive) [standard]: "
                )
                .strip()
                .lower()
            )
            ...
            if choice in self.setup_modes:
                break
            print(
                "Invalid choice. Please enter 'minimal', 'standard', or 'comprehensive'."
            )
        ...
        customized_config = self.customize_config_for_mode(
            template_config, selected_mode
        )
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--non-interactive":
        ...
        customized_config = installer.customize_config_for_mode(
            template_config, "standard"
        )
