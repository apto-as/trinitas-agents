...
class MemoryCreate(BaseModel):
    ...
    @validator('key', 'content', 'persona', 'category', pre=True)
...
async def create_memory(
    ...
    try:
        ...
        existing = await db.execute(
            select(Memory).where(Memory.key == memory_data.key)
        )
        ...
        memory = Memory(
            ...
            persona=memory_data.persona,
            category=memory_data.category,
            importance=memory_data.importance,
            is_shared=memory_data.is_shared,
            is_learned=memory_data.is_learned,
        ...
        logger.info(f"Memory created: {memory.key} by user {current_user.get('id')}")
...
async def search_memories(
    ...
    try:
        ...
        return MemoryListResponse(
            memories=[MemoryResponse.from_orm(memory) for memory in memories],
            total=total,
